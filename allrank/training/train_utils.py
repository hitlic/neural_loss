...
from allrank.models.model_utils import get_num_params, log_num_params  # pylint: disable=W0611
...
def loss_batch(model, loss_func, xb, yb, indices, gradient_clipping_norm, opt=None):
    ...
    pred_b = model(xb, mask, indices)  # 预测
    loss = loss_func(pred_b, yb)
...
def epoch_summary(epoch, train_loss, val_loss, train_metrics, val_metrics):
    summary = "Train loss: {train_loss:.8}  Val loss: {val_loss:.8}".format(
        train_loss=train_loss, val_loss=val_loss)
    for metric_name, metric_value in train_metrics.items():
        summary += "  Train {metric_name} {metric_value:.8}".format(
            metric_name=metric_name, metric_value=metric_value)

    for metric_name, metric_value in val_metrics.items():
        summary += "  Val {metric_name} {metric_value:.8}".format(
            metric_name=metric_name, metric_value=metric_value)
...
def fit(epochs, model, loss_func, optimizer, scheduler, train_dl, valid_dl, config,
        gradient_clipping_norm, early_stopping_patience, device, output_dir, tensorboard_output_path, logger, tag):
    # tensorboard_summary_writer = TensorboardSummaryWriter(tensorboard_output_path)

    num_params = get_num_params(model)
    # log_num_params(num_params)

    early_stop = EarlyStop(early_stopping_patience, logger, tag)
    ...
    for epoch in range(epochs):
        ...
        train_losses, train_nums = zip(
            *[loss_batch(model, loss_func, xb.to(device=device), yb.to(device=device), indices.to(device=device), gradient_clipping_norm, optimizer)
        ...
        with torch.no_grad():
            ...
            val_losses, val_nums = zip(
                *[loss_batch(model, loss_func, xb.to(device=device), yb.to(device=device), indices.to(device=device), gradient_clipping_norm)
                  for xb, yb, indices in valid_dl])
        ...
        logger.info(epoch_summary(epoch+1, train_loss, val_loss, train_metrics, val_metrics))

        current_val_metric_value = val_metrics.get(config.val_metric)
        if scheduler:
            if type(scheduler) == torch.optim.lr_scheduler.ReduceLROnPlateau:  # pylint: disable=C0123
                args = [val_metrics[config.val_metric]]
                scheduler.step(*args)
            else:
                scheduler.step()

        early_stop.step(current_val_metric_value, epoch, model)
        if early_stop.stop_training(epoch):
            ...
            break
    logger.info("load best ranking model")
    model.load_state_dict(early_stop.get_best_model())
    model.to(device)
    logger.epoch = None
