...
class LibSVMDataset(Dataset):
    ...
    def __init__(self, X, y, query_ids, transform=None):
        ...
        """
        if not isinstance(X, np.ndarray):  # 添加该行，当X是np.ndarray时不用再toarray
            X = X.toarray()
...
def create_test_dataloader(test_ds:LibSVMDataset, num_workers: int, batch_size: int):
    """创建测试dataloader"""
    gpu_count = torch.cuda.device_count()
    total_batch_size = max(1, gpu_count) * batch_size
    logger.info("total test batch size is {}".format(total_batch_size))
