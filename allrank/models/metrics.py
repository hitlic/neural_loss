...
def recall(y_pred, y_true, ats=None, padding_indicator=PADDED_Y_VALUE):
    """
    Recall at multiple ks (ats).

    Compute Recall at each rank given by ats with better efficiency.
    :param y_pred: predictions from the model, shape [batch_size, slate_length]
    :param y_true: ground truth labels, shape [batch_size, slate_length]
    :param ats: optional list of ranks for Recall evaluation, if None, maximum rank is used
    :param padding_indicator: an indicator of the y_true index containing a padded item, e.g. -1
    :return: Recall values for each slate and each evaluation position, shape [batch_size, len(ats)]
    """
    y_true = y_true.clone()
    y_pred = y_pred.clone()

    # If ats is None, we evaluate recall over the full length
    if ats is None:
        ats = [y_true.shape[1]]

    # Apply the padding mask: we mask out padded items in y_true
    mask = (y_true != padding_indicator).type(torch.float32)

    # Maximum k to compute Recall for
    max_k = max(ats)

    # Initialize a tensor to store recall results for each k in ats
    recalls = torch.zeros((y_true.shape[0], len(ats)), dtype=torch.float32, device=y_pred.device)

    # Compute total number of relevant items for each slate (excluding padding)
    total_relevant_items = torch.sum(y_true * mask, dim=1)


    # Sort y_pred in descending order and get top max_k indices only once
    _, topk_indices = torch.topk(y_pred, max_k, dim=1, largest=True, sorted=True)

    # Gather the true labels that correspond to the top max_k predictions
    topk_true = torch.gather(y_true, 1, topk_indices)

    # Apply both the padding mask and check if the true label is 1 (relevant item)
    relevant_items_in_topk = ((topk_true != padding_indicator) & (topk_true == 1)).type(torch.float32)

    # Cumulative sum of relevant items up to each position
    cumulative_relevant_items = torch.cumsum(relevant_items_in_topk, dim=1)

    # For each k in ats, compute Recall using cumulative sums
    for idx, k in enumerate(ats):
        retrieved_relevant_items = cumulative_relevant_items[:, k-1]  # Sum up to k items
        # Compute recall at this particular k, with division-by-zero protection
        recalls[:, idx] = torch.where(total_relevant_items > 0,
                                      retrieved_relevant_items / total_relevant_items,
                                      torch.zeros_like(total_relevant_items))
