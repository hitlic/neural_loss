...
def deterministic_neural_sort(s, tau, mask):
    ...
    n = s.size()[1]  # 文档数量（padding后）
    one = torch.ones((n, 1), dtype=torch.float32, device=dev)
    s = s.masked_fill(mask[:, :, None], -1e8)  # mask位置放一个极小的数字，使其排序后位于最后
    A_s = torch.abs(s - s.permute(0, 2, 1))    # 原文式(3)中的A_s
    A_s = A_s.masked_fill(mask[:, :, None] | mask[:, None, :], 0.0)  # mask位置对应的行、列都置0

    B = torch.matmul(A_s, torch.matmul(one, torch.transpose(one, 0, 1)))  # A_sI
